...
(function () {
    ...
    $(function () {
        $(".retail-model-btn").click(function () {
            alert(selectedid);
            if(selectedid === 0) return;
            $.ajax({
                type: "post",
                url: "/api/merchandises/retail",
                data: JSON.stringify(new RetailOrder(selectedid, 1, Number($(".retail-model-vipid").val()))),
        ...
        let render = function () {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                $(".merchandise-table tbody").append('<tr class="' + colorarray[i] + '">'
                    ...
                    +'<td><button class="btn btn-success retail-btn"  data-toggle="modal" data-target="#myModal" m_id='+merch_show[(cur_page - 1) * 5 + i].key.id+' >选择</button></td>'+
                    + '</tr>'
                );
            }
            $(".retail-btn").click(function () {
               selectedid = Number($(this).attr("m_id"));
                $(".retail-model-id").text(selectedid);
            });
