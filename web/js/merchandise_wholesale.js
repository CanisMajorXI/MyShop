(function () {
    ...
    $(function () {
        ...
        let render = function () {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                $(".merchandise-table tbody").append('<tr class="' + colorarray[i] + '">'
                    ...
                    + '<td>'
                    //   +'<button class="btn btn-success">添加到订单</button>'
                    + '<button class="btn btn-success add-button" merchandise-id=' + merch_show[(cur_page - 1) * 5 + i].key.id + '>添加到订单</button>'
                    + '</td>' +
                    +'</tr>');
            }
        ...
        $(".submit-batch-order-btn").click(function () {
                let idList = [];
                $.each(submit_List, function (index, value) {
